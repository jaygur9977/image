...
async function generateAIAnswer(text, purpose, answerType, projectDetails) {
  try {
    ...
    if (purpose === 'education') {
      if (answerType === 'project') {
        prompt += `Create a detailed project report with ${projectDetails.paragraphs} paragraphs and approximately ${projectDetails.words} words based on the following content:\n\n${text}`;
      } else if (answerType === 'long') {
        prompt += `Provide a detailed explanation (3-5 paragraphs) of the following educational content:\n\n${text}`;
      } else if (answerType === 'short') {
        prompt += `Provide a concise summary (2-3 sentences) of the following educational content:\n\n${text}`;
      } else if (answerType === 'one-word') {
        prompt += `What is the single most important word that summarizes this content? Content:\n\n${text}`;
      }
